...
| **工具** | Day 3 | 🔥 引入函数调用 (Tool Calling) — 对应文件 `3-c3-deepseek-tool.py` | ✅ **已发布** |
| | Day 4~6 | 多工具供调用，可以循环多轮次对话 | ✅ **已发布** |
| | Day 7~9 | 新增 edit / grep / webfetch 工具 | ✅ **已发布** |
| **增强** | Day 10~12 | 搜索 API 集成 |  ✅ **已发布** |
| | Day 13~14 | 🔥 读取经验记录 + 自动创建 | ✅ **已发布** |
| **高级** | Day 15 | 命令执行记录 |  **未发布** |
| | Day 16~17 | MCP 协议集成 (GitHub 工具) |  **未发布** |
| | Day 18~19 | 🔥 子 Agent 多团队协作 | ✅ **已发布** |
| | Day 20+ | 更多高级特性... | 🚀 |
...
## Day 13~14：读取经验记录 — Agent 的"成长日记"

**文件：** `7-deepseek-memory.py`

Agent 学会从 **经验记录.md** 文件读取过往的经验教训，并作为系统提示的一部分注入给大模型。这样 Agent 就能"站在前人的肩膀上"，避免重复犯错。

### 🧠 核心机制：经验即提示

```
启动时 → 读取 经验记录.md → 注入 system prompt → Agent 带着历史经验工作
```

| 行为 | 说明 |
|:--|:--|
| 📖 自动读取 | 启动时检测 `经验记录.md` 是否存在 |
| 📄 自动创建 | 若不存在，自动创建空文件（写入 `# 经验记录` 头部） |
| 🧩 注入提示 | 将文件内容拼入 system prompt，让模型知晓过往教训 |
| ✍️ 可写可改 | Agent 可用 `write_file` / `edit` 工具主动写入新经验 |
...
- 第一次运行自动创建 `经验记录.md`
- 之后每次启动都会读取已有记录
- 文件可手动编辑，也可由 Agent 自己通过工具追加

### 运行方式

```bash
python 7-deepseek-memory.py
```

### 📌 使用技巧

1. **手动注入经验**：提前在 `经验记录.md` 中写下已知问题，Agent 启动即知
2. **自动积累**：给 Agent 下达"每次任务完成后将经验写入 经验记录.md"的指令，让 Agent 自我成长
3. **多项目复用**：每个项目目录下放独立的 `经验记录.md`，实现项目专属经验
